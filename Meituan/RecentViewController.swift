...
class RecentViewController: BaseCollectionViewController,DealCellDelegate {
    ...
    @IBAction func delectItemClick(sender: UIBarButtonItem) {
        ...
        for deal in deals {
            ...
            if deal.checking ?? false {
                let fetchRquest = NSFetchRequest(entityName: "RecentDealsTable")
                fetchRquest.predicate = NSPredicate(format: "deal_id == %@", deal.deal_id)
                if let results = try! managedObjectContext.executeFetchRequest(fetchRquest) as? [RecentDealsTable] {
                    if results.count > 0 {
